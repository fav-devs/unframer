...
const DEFAULT_CLASS_PREFIX = process.env.REFRAME_CLASS_PREFIX || 'unframer'
const RUNTIME_LOCAL_PATH = process.env.REFRAME_RUNTIME_PATH || ''
const REWRITE_IMPORT_TO = process.env.REFRAME_REWRITE_IMPORT_TO || ''
...
export async function bundle({
    ...
    const buildContext = await fn({
        ...
        external: [...defaultExternalPackages, (REWRITE_IMPORT_TO || RUNTIME_PACKAGE_NAME)].filter(Boolean) as any,
    ...
    const doNotEditComment = GENERATED_BANNER_ENABLE ? `/* This file was generated by ${GENERATED_BANNER_BRAND} for Framer project ${config.projectId || ''} "${config.projectName}", do not edit manually */
` : ''
    ...
    async function rebuild() {
        ...
        try {
            ...
            if (
                ...
                if (!SUPPRESS_INSTALL) { missingPackages.add(`${RUNTIME_PACKAGE_NAME}@${currentUnframerVersion}`) }
                spinner.info(
                    `Different runtime detected (), will install @`,
                )
            }
        } catch (e) {
...
/*
async function scrubOutDir(dir: string, emittedPkg: string, rewriteTo: string, bannerEnabled: boolean) {
  try {
    const entries = await fs.promises.readdir(dir, { withFileTypes: true })
    for (const ent of entries) {
      const full = path.join(dir, ent.name)
      if (ent.isDirectory()) { await scrubOutDir(full, emittedPkg, rewriteTo, bannerEnabled); continue }
      if (!/(\.jsx?|\.tsx?|\.css)$/i.test(ent.name)) continue
      let text = await fs.promises.readFile(full, 'utf-8')
      if (rewriteTo && rewriteTo !== emittedPkg) {
        text = text.replace(/from\s+['"]unframer['"]/g, `from '${rewriteTo}'`)
        if (emittedPkg && emittedPkg !== 'unframer') {
          const rx = new RegExp(`from\s+['"]${emittedPkg}['"]`, 'g')
          text = text.replace(rx, `from '${rewriteTo}'`)
        }
      }
      if (!bannerEnabled) {
        text = text.replace(/\/\* This file was generated by[\s\S]*?do not edit manually \*\/?
?/g, '')
      }
